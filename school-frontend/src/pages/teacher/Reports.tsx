...
export default function TeacherClassReport() {
  ...
  useEffect(() => {
    axios.get('/api/v1/teachers/my-classes', { headers })
  ...
  const fetchData = async () => {
    ...
    setLoading(true)
    try {
      const res = await axios.get(`/api/v1/reports/class/${selectedClassId}`, { headers })
  ...
  const exportFile = async (type: 'csv' | 'pdf') => {
    ...
    try {
      const res = await axios.get(`/api/v1/reports/export/class/${selectedClassId}/${ext}`, {
        headers,
        responseType: 'blob',
      })
