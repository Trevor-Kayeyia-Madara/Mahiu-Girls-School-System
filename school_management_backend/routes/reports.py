...
def export_student_pdf(current_user, student_id):
    # 🔐 Authorization: allow admin, teacher, or the student
    student = Student.query.get_or_404(student_id)
    if current_user.role not in ['admin', 'teacher'] and current_user.user_id != getattr(student.user, 'user_id', None):
        return jsonify({'error': 'Access denied'}), 403

    # 📊 Get Grades
    grades = Grade.query.filter_by(student_id=student_id).join(Subject).join(Exam).all()
...
def export_student_csv(current_user, student_id):
    ...
    for g in grades:
        writer.writerow([g.subject.name, g.exam.name, g.score])
...
def export_class_csv(current_user, class_id):
    ...
    output = BytesIO()
    writer = csv.writer(output)
    writer.writerow(['Student Name', 'Subject', 'Score', 'Grade', 'Exam'])

    for s in students:
        grades = Grade.query.filter_by(student_id=s.student_id).join(Subject).join(Exam).all()
        for g in grades:
            writer.writerow([
