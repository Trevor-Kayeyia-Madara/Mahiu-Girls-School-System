...
def student_report(current_user, student_id):
    ...
    grades = Grade.query.filter_by(student_id=student_id).options(joinedload(Grade.subject)).all()
    if not grades:
    ...
    return jsonify({
        ...
        'class_name': student.classroom.class_name if student.classroom else 'N/A',
